...
build_windows:
	...
	gogio -ldflags="-X version.AppVersion=${TAG_NAME}" -target=windows -arch=amd64 -o "dist\amd64\Chapar.exe" .
	gogio -ldflags="-X version.AppVersion=${TAG_NAME}" -target=windows -arch=386 -o "dist\i386\Chapar.exe" .
	gogio -ldflags="-X version.AppVersion=${TAG_NAME}" -target=windows -arch=arm64 -o "dist\arm64\Chapar.exe" .
	rm *.syso
...
build_linux:
	...
	CGO_ENABLED=1 GOOS=linux GOARCH=amd64 go build -ldflags="-X version.AppVersion=$(TAG_NAME)" -o ./dist/amd64/chapar .
